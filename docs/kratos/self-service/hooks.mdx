...
In some cases you might want to cancel a webhook and not make the request. A
good example might be when you test your systems and do not want the test
accounts to end up in your e.g. CRM. In this case you can cancel the web-hook by
raising an error:
...
Currently, JsonNet doesn't support regular expressions. 
Follow this issue to see if the feature has been implemented:
